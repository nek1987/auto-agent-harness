...
def register_project(name: str, path: Path, force: bool = False) -> None:
    """
    ...
    Args:
        ...
        force: If True, overwrite existing registration if path doesn't exist.

    Raises:
        ValueError: If project name is invalid or path is not absolute.
        RegistryError: If a project with that name already exists (and force=False).
    """
    # Validate name
    ...
    with _get_session() as session:
        ...
        if existing:
            ...
            # If existing path doesn't exist anymore, allow re-registration
            if not existing_path.exists():
                logger.info(
                    "Project '%s' has stale path '%s', updating to '%s'",
                    name, existing.path, path
                )
                existing.path = path.as_posix()
                existing.created_at = datetime.now()
                logger.info("Updated project '%s' to path: %s", name, path)
                return

            # If force and paths are different, update
            if force and existing_path != path:
                existing.path = path.as_posix()
                existing.created_at = datetime.now()
                logger.info("Force-updated project '%s' to path: %s", name, path)
                return

            # If same path, it's a no-op
            if existing_path == path:
                logger.debug("Project '%s' already registered at same path", name)
                return
